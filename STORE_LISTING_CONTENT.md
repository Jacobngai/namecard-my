# WhatsCard - App Store Listing Content

## 📱 App Information

**App Name:** WhatsCard - Business Card Scanner
**Short Name:** WhatsCard
**Package/Bundle ID:** com.whatscard.app
**Version:** 1.0.0
**Category:** Business / Productivity

---

## 📝 Store Descriptions

### Short Description (80 characters max)
Smart business card scanner with instant WhatsApp networking

### Full Description (Google Play - 4000 char max, App Store - no limit)

**Transform Paper Cards into Digital Connections**

WhatsCard is your smart networking companion that digitizes business cards instantly and helps you build meaningful professional relationships. Scan, save, and connect with anyone on WhatsApp in seconds.

**🚀 KEY FEATURES**

**Smart OCR Technology**
• Scan business cards with AI-powered accuracy
• Automatic contact information extraction
• Support for multiple languages and card formats
• Works offline - no internet required for scanning

**Seamless Contact Management**
• Auto-organize contacts by industry, location, or custom tags
• Advanced search and filtering
• Quick access to contact history and notes
• One-tap WhatsApp messaging

**WhatsApp Integration**
• Instant connection via WhatsApp
• Send personalized follow-up messages
• Share your digital business card
• Network smarter, not harder

**Professional Features**
• Export contacts to Excel/CSV
• Bulk contact operations
• Dark mode support
• Offline-first architecture - works anywhere

**Security & Privacy**
• Your data stays secure with encryption
• Optional cloud backup via Supabase
• GDPR compliant
• No ads, no tracking

**💼 PERFECT FOR**
• Business professionals
• Sales representatives
• Event attendees
• Entrepreneurs
• Recruiters
• Real estate agents
• Anyone who networks!

**📊 SUBSCRIPTION TIERS**

**Free Plan**
• Unlimited card scanning
• Basic contact management
• WhatsApp integration
• Excel export

**Pro Plan (RM199/year)**
• All Free features
• Follow-up reminders
• Voice notes with AI transcription
• Advanced analytics
• Priority support

**Enterprise Plan (RM599/year)**
• All Pro features
• Team collaboration
• Custom integrations
• Dedicated account manager
• White-label options

**🌟 WHY WHATSCARD?**

Join thousands of professionals who have ditched the shoebox of business cards and embraced digital networking. WhatsCard helps you:

✓ Never lose a valuable connection
✓ Follow up faster than your competition
✓ Build lasting professional relationships
✓ Stay organized effortlessly
✓ Network with confidence

Download WhatsCard today and turn every introduction into an opportunity!

**💬 CONTACT & SUPPORT**
• Email: support@whatscard.app
• Website: https://whatscard.app
• Privacy Policy: https://whatscard.app/privacy-policy
• Terms of Service: https://whatscard.app/terms

**Note:** WhatsApp integration requires WhatsApp to be installed on your device.

---

## 🏷️ Keywords (Max 100 characters for App Store)

business card scanner, networking, OCR, contact management, WhatsApp, CRM, business contacts, card reader, professional networking, sales tool

### Google Play Keywords (Comma-separated)
business card, scanner, OCR, contact management, WhatsApp, networking, CRM, business networking, card scanner, professional contacts, sales CRM, contact organizer, business contacts, digital business card, card reader app

---

## 📸 Screenshot Requirements

### Android (Google Play)
- **Minimum:** 2 screenshots
- **Maximum:** 8 screenshots
- **Size:** 1080x1920 (portrait) or 1920x1080 (landscape)
- **Format:** PNG or JPEG

**Recommended Screenshots:**
1. Home screen with contact list
2. Camera scanning a business card
3. Contact detail view with WhatsApp button
4. Excel export feature
5. Search and filter functionality
6. Dark mode example
7. Voice notes feature (Pro)
8. Analytics dashboard (Pro)

### iOS (App Store)
- **Minimum:** 1 screenshot per device size
- **6.5" Display:** 1284x2778 (iPhone 14 Pro Max)
- **5.5" Display:** 1242x2208 (iPhone 8 Plus)
- **12.9" iPad:** 2048x2732
- **Format:** PNG or JPEG

**Same 8 screenshots as Android**

---

## 🎨 App Icon Requirements

### Android
- **512x512 PNG** (Google Play listing)
- **Adaptive Icon:** 512x512 PNG (foreground + background)
- Must not have alpha channel

### iOS
- **1024x1024 PNG** (App Store listing)
- Must be exactly 1024x1024 pixels
- No alpha channel
- No rounded corners (iOS handles this)

**Current icon:** `./assets/splash-icon.png`
**Status:** ⚠️ Needs verification for store requirements

---

## 📊 Content Rating

### Google Play - Questionnaire Answers
- **Violence:** None
- **Sexual Content:** None
- **Language:** None
- **Controlled Substances:** None
- **Discrimination:** None
- **Data Collection:** Contact information (for app functionality)
- **Target Age:** 18+

**Expected Rating:** PEGI 3 / Everyone

### iOS App Store
- **Age Rating:** 4+
- **Content Descriptors:** None
- **Gambling:** No
- **Contests:** No
- **Unrestricted Web Access:** No

---

## 🌍 Availability

### Countries/Regions
- **Primary:** Malaysia
- **Expansion:** Singapore, Indonesia, Thailand, Philippines
- **Global:** All countries (English support)

### Pricing
- **Base Price:** Free (with in-app purchases)
- **Pro Subscription:** RM199/year
- **Enterprise Subscription:** RM599/year

---

## 🔐 Privacy Policy & Terms

### Privacy Policy URL
https://whatscard.app/privacy-policy

**Key Points:**
- We collect: Name, email, phone, business card images
- Purpose: Contact management and app functionality
- Storage: Secure cloud backup (optional)
- Sharing: No third-party data sharing
- User rights: Data export, deletion, opt-out

### Terms of Service URL
https://whatscard.app/terms

**Key Points:**
- Subscription terms and billing
- Refund policy
- Acceptable use policy
- Intellectual property rights

---

## 📧 Support & Contact

- **Support Email:** support@whatscard.app
- **Website:** https://whatscard.app
- **Marketing Website:** https://whatscard.app
- **Privacy Policy:** https://whatscard.app/privacy-policy
- **Terms of Service:** https://whatscard.app/terms

---

## 📝 Promotional Text (170 characters for App Store)

🚀 NEW: Voice notes with AI transcription! Record follow-up notes instantly after meetings. Upgrade to Pro and never forget an important detail again!

---

## 🎬 App Preview Video (Optional but Recommended)

**Duration:** 15-30 seconds
**Format:** MP4 or MOV
**Resolution:** 1080x1920 (portrait)

**Script:**
1. Open app → Scan business card
2. AI extracts contact info automatically
3. One-tap WhatsApp connection
4. Show contact management features
5. Export to Excel
6. CTA: "Download WhatsCard - Network Smarter"

---

## ✅ Pre-Launch Checklist

### Before Submission
- [ ] All screenshots taken (8 per platform)
- [ ] App icon meets requirements (1024x1024 iOS, 512x512 Android)
- [ ] Privacy policy published at whatscard.app/privacy-policy
- [ ] Terms of service published at whatscard.app/terms
- [ ] Support email active (support@whatscard.app)
- [ ] Test app on multiple devices
- [ ] Fix all critical bugs
- [ ] Complete content rating questionnaire
- [ ] Set up in-app purchases in store consoles
- [ ] Configure subscription products
- [ ] Set pricing for all regions

### Google Play Console
- [ ] Developer account created ($25)
- [ ] App created in console
- [ ] Store listing completed
- [ ] Content rating obtained
- [ ] Pricing & distribution set
- [ ] App bundle (.aab) uploaded
- [ ] Release notes written
- [ ] Production release created

### App Store Connect
- [ ] Apple Developer account ($99/year)
- [ ] App created in App Store Connect
- [ ] Store listing completed
- [ ] Age rating configured
- [ ] Pricing & availability set
- [ ] Build uploaded (.ipa via EAS)
- [ ] Screenshots uploaded
- [ ] Submit for review

---

## 🚀 Launch Day Checklist

- [ ] Final build submitted
- [ ] Monitor review status
- [ ] Prepare launch announcement
- [ ] Update website
- [ ] Social media posts ready
- [ ] Email marketing campaign ready
- [ ] Press release prepared
- [ ] Customer support ready

---

## 📱 Post-Launch Monitoring

### Week 1
- [ ] Monitor crash reports
- [ ] Check user reviews
- [ ] Respond to support tickets
- [ ] Track download numbers
- [ ] Monitor subscription conversions

### Month 1
- [ ] Analyze user feedback
- [ ] Plan feature updates
- [ ] Optimize app store listing
- [ ] A/B test screenshots
- [ ] Update promotional text

---

**Last Updated:** January 2025
**Version:** 1.0.0
**Status:** Pre-launch preparation
