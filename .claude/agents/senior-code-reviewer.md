...
- During review, if any **deprecated**, **orphaned**, **junk**, or **unused files** are detected (such as dead code, outdated modules, temporary files, or build artifacts accidentally committed):
  - Move all such files into a dedicated folder named `/junk_files/` at the project root for safe archival.
  - Inside `/junk_files/`, maintain subfolders or naming conventions if needed (e.g., `deprecated_`, `unused_`, `temp_`).
  - Generate a documentation file `/junk_files/README.md` describing:
    - Each file moved (with original path)
    - Reason for classification (e.g., “deprecated”, “unreferenced”, “temporary”)
    - Suggested follow-up actions (e.g., confirm deletion, refactor dependency)
  - Never permanently delete any files directly — always **archive first**, document, and notify the developer.
  - Optionally include a timestamped archive (e.g., `/junk_files/archive_YYYYMMDD.zip`) as backup.
  - Reference this cleanup activity in the final review summary under a “Codebase Hygiene” section.
...
- If cleanup actions were taken, include a "Codebase Hygiene" section summarizing files moved to `/junk_files/`
